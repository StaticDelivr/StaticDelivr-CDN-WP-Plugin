...
class StaticDelivr_Images
{
        ...
        if (strpos($original_url, 'cdn.staticdelivr.com') !== false) {
            ...
            if (strpos($original_url, '/img/images?') !== false && strpos($original_url, 'url=') !== false) {
                ...
                if (preg_match('#cdn\.staticdelivr\.com/img/(.+)$#', $original_url, $matches)) {
                    ...
                    // Attempt to find the attachment URL by filename pattern
                    $recovered_url = $this->find_attachment_url_by_filename($filename);

                    if ($recovered_url) {
                        $original_url = $recovered_url;
                        $this->debug_log('Recovered original URL from attachment: ' . $original_url);
                    } else {
                        // Fallback: Try to reconstruct using upload dir (current year/month) if DB lookup fails
                        // This is a last resort and might fail for older images, but better than nothing
                        $upload_dir = wp_upload_dir();
                        $original_url = $upload_dir['baseurl'] . '/' . date('Y/m') . '/' . $filename;
                        $this->debug_log('Could not find attachment in DB, trying date-based reconstruction: ' . $original_url);
                    }
     ...
     *
     * Searches the WordPress attachment database for a file matching the given filename.
     * Used to recover original URLs from malformed CDN URLs.
     *
     * @param string $filename The filename to search for.
     * @return string|false The attachment URL if found, false otherwise.
     */
    private function find_attachment_url_by_filename($filename)
    {
        global $wpdb;

        // Remove any dimension suffix (e.g., -600x400) to get the base filename.
        // This handles cases where the CDN URL includes dimensions.
        $base_filename = preg_replace('/-\d+x\d+(\.[^.]+)$/', '$1', $filename);

        // Search for attachment by filename in the database (efficient LIKE query on indexed meta_value isn't perfect but works for paths).
        // Note: _wp_attached_file stores relative path like '2025/12/image.jpg'.
        // We match against the filename part.
